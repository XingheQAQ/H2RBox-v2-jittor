...
class PSCCoder:
    ...
    def __init__(self,
        ...
        self.coef_sin = jt.array([
            math.sin(2 * k * math.pi / self.num_step) 
            for k in range(self.num_step)
        ]).float()
        self.coef_cos = jt.array([
            math.cos(2 * k * math.pi / self.num_step) 
            for k in range(self.num_step)
        ]).float()
    ...
    def encode(self, angle_targets):
        ...
        参数:
            angle_targets (jt.Var): 角度目标值(弧度)，形状为[N, 1]

        返回:
            jt.Var: 编码后的相位偏移模式，形状为[N, encode_size]
        """
        # 转换为相位角
        ...
        # 双频编码处理方形目标问题
        if self.dual_freq:
            phase_targets = angle_targets * 4  # 高频分量
            phase_shift_targets += [
                jt.cos(phase_targets + 2 * math.pi * x / self.num_step)
                for x in range(self.num_step)
            ]
    ...
    def decode(self, angle_preds, keepdim=False):
        ...
        # 低频分量解码
        phase_sin = jt.sum(
            angle_preds[..., :self.num_step] * self.coef_sin,
            dim=-1,
            keepdims=keepdim)
        phase_cos = jt.sum(
            angle_preds[..., :self.num_step] * self.coef_cos,
            dim=-1,
            keepdims=keepdim)
        phase_mod = phase_cos**2 + phase_sin**2
        phase = -jt.atan2(phase_sin, phase_cos) # [-pi,pi)

        # 双频解码处理
        if self.dual_freq:
            phase_sin = jt.sum(
                angle_preds[..., self.num_step:(2*self.num_step)] * self.coef_sin,
                dim=-1,
                keepdims=keepdim)
            phase_cos = jt.sum(
                angle_preds[..., self.num_step:(2*self.num_step)] * self.coef_cos,
                dim=-1,
                keepdims=keepdim)
            ...
            # 相位解缠绕处理
            cos_diff = jt.cos(phase)*jt.cos(phase2) + jt.sin(phase)*jt.sin(phase2)
            phase2 = jt.where(cos_diff < 0, phase2 % (2*math.pi) - math.pi, phase2)
            phase = phase2

        # 过滤各向同性目标
        angle_pred = phase * (phase_mod >= self.thr_mod) / 2
